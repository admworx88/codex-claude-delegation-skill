...
Test-Path (Join-Path $destination 'references\security-model.md')
Test-Path (Join-Path $destination 'agents\openai.yaml')
```

Success prints `True` six times.
...
test -f "$destination/references/security-model.md" \
  && echo "Security model found"
...
`Result schema found`, `Security model found`, and
`Codex interface manifest found`.
...
Release and same-repository CI runs also pass `-RequireLiveEnforcement`. Add a
repository Actions secret named `CLAUDE_CODE_OAUTH_TOKEN`, generated by the
owner with `claude setup-token`; the live job fails closed when that credential
is absent or when any real deny-rule, direct-mode, or subagent-inheritance probe
does not run. Fork pull requests run only the non-authenticated contract suite
because GitHub does not expose repository secrets to forks.
